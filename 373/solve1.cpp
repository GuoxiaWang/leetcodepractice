...
public:
    struct Comp {
        int operator() (const pair<pair<int, int>, int>& lhs, const pair<pair<int, int>, int>& rhs) {
    ...
    vector<pair<int, int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k) {
        ...
        for (int i=0; i<n && i<k; i++) {
            q.push(make_pair(make_pair(i, 0), nums1[i] + nums2[0]));
        }
        while (k-- && !q.empty()) {
            ...
            if (j == m-1) continue;
            q.push(make_pair(make_pair(i, j+1), nums1[i] + nums2[j+1]));
        }
        return ans;
    }
};
